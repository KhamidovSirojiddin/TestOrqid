<?php
if(!class_exists("StukramPortfolioWordPressThemeBase")) {
	class StukramPortfolioWordPressThemeBase {
    	public $key = "E732121458DE5631";
    	private $product_id = "49";
    	private $product_base = "stukram";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_stukram_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=stukram_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
